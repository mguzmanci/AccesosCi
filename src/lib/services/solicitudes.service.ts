...
export function validarEntradaSolicitud(input: NuevaSolicitudInput): string[] {
  ...
  if (input.tipo === "crear") {
    ...
    if (esVacio(d.nombres)) errores.push("Los nombres son obligatorios.");
    if (esVacio(d.apellidos)) errores.push("Los apellidos son obligatorios.");
    if (esVacio(d.telefono)) errores.push("El teléfono es obligatorio.");
    if (esVacio(d.correoPersonal)) {
      errores.push("El correo personal es obligatorio.");
    }
