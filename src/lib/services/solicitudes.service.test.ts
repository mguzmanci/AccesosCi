...
describe("validarEntradaSolicitud", () => {
  ...
  it("exige nombres y apellidos al crear", () => {
    const errores = validarEntradaSolicitud({
      ...inputCrear,
      datos: { nombres: "", apellidos: "", telefono: "", correoPersonal: "" },
    });
  ...
  it("exige correo corporativo del solicitante", () => {
    ...
    expect(errores).toContain(
      "El solicitante debe usar un correo @capitalinteligente.cl.",
    );
  });
...
describe("construirDirectorio", () => {
  ...
  it("ordena el directorio alfabéticamente por correo corporativo", () => {
    const b = solicitudCrearCompletada("b@capitalinteligente.cl", ["gmail"], "2026-01-01T00:00:00.000Z");
    const a = solicitudCrearCompletada("a@capitalinteligente.cl", ["gmail"], "2026-01-02T00:00:00.000Z");
